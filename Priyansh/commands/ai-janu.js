...
const handleEvent = async function ({ api, event, client, __GLOBAL }) {
  ...
  if (event.body.indexOf("Janu") === 0 || event.body.indexOf("JANU") === 0 || event.body.indexOf("janu") === 0 || event.body.indexOf("janu") === 0) {
    const { threadID, messageID } = event;
    const input = event.body;
    const message = input.split(" ");

    if (message.length < 2) {
      api.sendMessage("✨ 𝙷𝚎𝚕𝚕𝚘, Type✍🏻 Janu aur Apna question pucho", threadID);
    } else {
      try {
